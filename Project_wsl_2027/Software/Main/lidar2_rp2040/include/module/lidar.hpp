#pragma once

#include <Arduino.h>
#include "common/serial_packet.hpp"
#include "common/timer.hpp"
#include "common/const_number.hpp"

namespace lidar
{
    const uint8_t packet_len = 47;       // パケットの長さ
    const uint8_t point_per_packet = 12; // 1パケットが受信するデータ量
    const uint8_t header = 0x54;         // ヘッダーコード

    inline uint16_t dis[360];                // 距離 y軸正の向きから反時計回りに格納
    inline uint8_t cycle = 0;                // point_per_packet回データを受け取ると0に戻る
    inline uint8_t prev_cycle = 0;           // point_per_packet回データを受け取ると0に戻る
    inline HardwareSerial *serial = nullptr; // シリアル

    // エラーチェックテーブル
    static const uint8_t crc_table[256] = {
        };

    inline uint8_t calcCRC8(const uint8_t *data, uint16_t len) // エラーチェック
    {
        uint8_t crc = 0;
        for (uint16_t i = 0; i < len; i++)
        {
            crc = crc_table[(crc ^ data[i]) & 0xff];
        }
        return crc;
    }

    inline void parsePacket(uint8_t *p) // データ読み取り
    {
        uint16_t start_angle_raw = (p[5] << 8) | p[4];
        uint16_t end_angle_raw = (p[43] << 8) | p[42];

        float sa = start_angle_raw / 100.0;
        float ea = end_angle_raw / 100.0;
        if (ea < sa)
            ea += 360.0;

        for (int i = 0; i < point_per_packet; i++)
        {
            int idx = 6 + i * 3;
            uint16_t distance = (p[idx + 1] << 8) | p[idx];

            float angle_float = sa + (ea - sa) * (i / 11.0);
            if (angle_float >= 360.0)
                angle_float -= 360.0;

            int angle_bucket = (int)(angle_float + 0.5);
            if (angle_bucket >= 360)
                angle_bucket = 0;

            if (distance >= 50 && distance <= 12000)
            {
                dis[359 - angle_bucket] = distance;
            }
        }

        prev_cycle = cycle;
        cycle++;
    }

    inline void attach(HardwareSerial &serial_obj) // どのシリアルで通信するか紐づけ
    {
        serial = &serial_obj;

        for (int i = 0; i < 360; i++)
        {
            dis[i] = 0;
        }
    }

    inline void process() // Lidarとの通信
    {
        if (!serial)
            return; // ヌルポインタ安全対策

        static uint8_t buf[packet_len];
        static uint8_t idx = 0;
        static bool receiving = false;

        while (serial->available())
        {
            uint8_t b = (uint8_t)serial->read();

            if (!receiving)
            {
                if (b == header)
                {
                    buf[0] = b;
                    idx = 1;
                    receiving = true;
                }
                continue;
            }

            buf[idx++] = b;

            if (idx >= packet_len)
            {
                receiving = false;
                idx = 0;

                if (buf[1] != 0x2C)
                {
                    continue;
                }

                uint8_t crc = calcCRC8(buf, packet_len - 1);
                if (crc != buf[packet_len - 1])
                {
                    continue;
                }
                parsePacket(buf);
            }
        }
    }
}